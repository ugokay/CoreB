<template>
  <div class="full-height">
    ...
    <div class="full-height" v-else>
      <div class="editing-mode-tools">
        <a v-if="!isEditingMode" flat block class="mt-0" @click="$router.push(designLink)">{{ element.title }}</a>
        <input v-else type="input" v-model="element.title"/>
        <v-menu offset-y>
      ...
      <div class="report-element-wrapper-outer" ref="elementWrapper" v-if="queryResult.schema">
        <template v-if="element.chartType === 6">
